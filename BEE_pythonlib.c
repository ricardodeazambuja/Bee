...
---------------------------------
BEE - Spiking Reservoir Simulator
----------------------------------
...
gcc -shared -Wl, -install_name, file_name.so -o file_name.so -fPIC file_name.c -O3 -lpthread -Wall -std=gnu99 -ffast-math random_np/mtrand/randomkit.c random_np/mtrand/distributions.c -mtune=native

When some of the information is not available it's possible to save to a file (missed.all) with this option: -fopt-info-missed=missed.all

My laptop (OSX corei5):
gcc my_simulator_pthreads_opt.c -o my_simulator_pthreads_opt2.o -lpthread -O3 -std=gnu99 -Wall -ffast-math -mtune=native random_np/mtrand/randomkit.c random_np/mtrand/distributions.c

Desktop (Linux corei7):
gcc my_simulator_pthreads_opt.c -o my_simulator_pthreads_opt2.o -lpthread -O3 -std=gnu99 -Wall -ffast-math -mtune=native -lrt -lm random_np/mtrand/randomkit.c random_np/mtrand/distributions.c

In order to compile it in Linux it is necessary to link the "math.h" manually including "-lm" and "-lrt"
...
#include "random_np/mtrand/randomkit.h"
#include "random_np/mtrand/distributions.h"
...
//
float const SpkLiq_liquid_parametres[2][2][6] = {{
...
float SpkLiq_vresets; // stores the voltage value the membrane must assume after a reset (refractory or spike) (in Volts)
...
int *restrict SpkLiq_pre_i; //stores the indices of the inhibitory=>? pre-synaptic connections in the liquid
int *restrict SpkLiq_pos_i; //stores the indices of the inhibitory=>? pos-synaptic connections in the liquid
float *restrict SpkLiq_w_i; //stores the weights of the connections above

int *restrict SpkLiq_pre_e; //stores the indices of the inhibitory=>? pre-synaptic connections in the liquid
int *restrict SpkLiq_pos_e; //stores the indices of the inhibitory=>? pos-synaptic connections in the liquid
float *restrict SpkLiq_w_e; //stores the weights of the connections above
    ...
    */
    ...
    { //These are the PRE indices
        ...
        { //These are the POS indices
            ...
            float const prob_dist = (SpkLiq_liquid_parametres[PRE][POS][0]*exp(-(pow((a_3Didx_ptr[indices_ptr[i]].x-a_3Didx_ptr[indices_ptr[j]].x),2)+pow((a_3Didx_ptr[indices_ptr[i]].y-a_3Didx_ptr[indices_ptr[j]].y),2)+pow((a_3Didx_ptr[indices_ptr[i]].z-a_3Didx_ptr[indices_ptr[j]].z),2))*lbd_value_opt));
            int creation_test = 0;
            ...
            if (creation_test)
            {
                ...
                {//Inhibitory=>? connection
                 ...
                 // SpkLiq_w_i_thread_internal[(*SpkLiq_inh_connections_thread_internal)-1]=1E-9*SpkLiq_liquid_parametres[PRE][POS][4]*fabs((float)rk_gauss(&SpkLiq_threads_states[3][SpkLiq_tid])*0.5+1); //AMaass
                    SpkLiq_w_i_thread_internal[(*SpkLiq_inh_connections_thread_internal)-1]=1E-9*SpkLiq_liquid_parametres[PRE][POS][4]*fabs((float)rk_normal(&SpkLiq_threads_states[3][SpkLiq_tid],0,1)*0.5+1); //AMaass
                ...
                {//Excitatory=>? connections
                 ...
                 // SpkLiq_w_e_thread_internal[(*SpkLiq_exc_connections_thread_internal)-1]=1E-9*SpkLiq_liquid_parametres[PRE][POS][4]*fabs((float)rk_gauss(&SpkLiq_threads_states[3][SpkLiq_tid])*0.5+1); //AMaass
                    SpkLiq_w_e_thread_internal[(*SpkLiq_exc_connections_thread_internal)-1]=1E-9*SpkLiq_liquid_parametres[PRE][POS][4]*fabs((float)rk_normal(&SpkLiq_threads_states[3][SpkLiq_tid],0,1)*0.5+1); //AMaass
                }
            }
...
void *SpikingLiquid_update_internal_thread(void *args)
{
  ...
  */
    ...
    const float SpkLiq_vresets_internal = SpkLiq_vresets;
...
/////FAST SET BIT ON
            ...
            SpkLiq_neurons_membrane_internal[i] = SpkLiq_vresets_internal; //# Resets the membrane of the spiked ones
...
int parse_sim_config(char *file_name)
{
    ...
    while (fscanf(config_file, "%s\n", str_read) != EOF)
    {
        ...
        {
              // float SpkLiq_vresets;
              fscanf(config_file, "%s\n", str_read);
              SpkLiq_vresets = atof(str_read);
              printf("#SpkLiq_vresets:%3.2E\n",SpkLiq_vresets);
              continue;
        }
