...
liquid_writes_initial_voltages = SNNSIM.writes_membranes_init
liquid_writes_initial_voltages.restype = None
liquid_writes_initial_voltages.argtypes = [numpy.ctypeslib.ndpointer(dtype=numpy.float32, flags='ALIGNED,C_CONTIGUOUS')]
...
liquid_writes_exc_indices = SNNSIM.writes_excitatory_indices
liquid_writes_exc_indices.restype = None
liquid_writes_exc_indices.argtypes = [numpy.ctypeslib.ndpointer(dtype=numpy.int32, flags='ALIGNED,C_CONTIGUOUS')]
...
liquid_writes_inh_indices = SNNSIM.writes_inhibitory_indices
liquid_writes_inh_indices.restype = None
liquid_writes_inh_indices.argtypes = [numpy.ctypeslib.ndpointer(dtype=numpy.int32, flags='ALIGNED,C_CONTIGUOUS')]
...
liquid_writes_noisy_offset = SNNSIM.writes_noisy_offset_currents
liquid_writes_noisy_offset.restype = None
liquid_writes_noisy_offset.argtypes = [numpy.ctypeslib.ndpointer(dtype=numpy.float32, flags='ALIGNED,C_CONTIGUOUS')]
...
liquid_writes_refrac_values = SNNSIM.writes_refrac_values
liquid_writes_refrac_values.restype = None
liquid_writes_refrac_values.argtypes = [numpy.ctypeslib.ndpointer(dtype=numpy.float32, flags='ALIGNED,C_CONTIGUOUS')]
...
def writes_initial_voltages(membrane_init):
    '''
    Writes the initial value used for the membrane voltages (numpy array float32).
    Same size as total number of neurons.
    '''
    if BEE_initialized():
...
def writes_noisy_offsets(noisy_offsets):
    '''
    Writes the values of the fixed offset noisy currents (numpy array float32).
    Same size as total number of neurons.
    '''
    if BEE_initialized():
...
def writes_refrac_values(refrac_values):
    '''
    Writes the values of the refractory periods (numpy array float32).
    Same size as total number of neurons.
    '''
    if BEE_initialized():
...
def writes_exc_indices(exc_indices):
    '''
...
def reads_exc_indices():
    '''
    Reads the array that controls which neurons are excitatory.
    '''
    if BEE_initialized():
        output = numpy.empty(read_SpkLiq_number_of_exc_neurons(),dtype=numpy.int32)
        liquid_exc_indices(output)
        return output
    else:
        print "Simulator is not ready!"
...
def reads_inh_indices():
    '''
    Reads the array that controls which neurons are inhibitory.
    '''
    if BEE_initialized():
        output = numpy.empty(read_SpkLiq_number_of_inh_neurons(),dtype=numpy.int32)
        liquid_inh_indices(output)
        return output
    else:
        print "Simulator is not ready!"
